"""
Full Modal Deployment with Complete Detection Logic
All imports inside functions to avoid Modal's local validation
"""
import modal
...
def detect(file_bytes: bytes, video_id: str = "", user_id: str = "", include_details: str = "false"):
    """Main detection endpoint with full ensemble logic"""
    ...
    if DEEPFACE_AVAILABLE:
        try:
            ...
            demographics = {
                ...
                "dominant_gender": demo.get("dominant_gender", ""),
                "race": demo.get("race", {}),
                "dominant_race": demo.get("dominant_race", ""),
                "emotion": demo.get("emotion", {}),
                "dominant_emotion": demo.get("dominant_emotion", "")
            }
        except Exception as e:
    ...
    for name, model_data in models.items():
        try:
            ...
            predictions[name] = {
                ...
                'is_fake': weighted_prob > THRESHOLD,
                'adjustment': float(adjustment)
            }
            
            logger.info(f"{name}: {raw_prob:.4f} → {weighted_prob:.4f} (adj: {adjustment:+.4f})")
    ...
    logger.info(f"Ensemble: {ensemble_score:.4f} → {'DEEPFAKE' if is_deepfake else 'REAL'}")
...
def fastapi_app():
    ...
    web_app = FastAPI(title="Deepfake Detection API", docs_url="/docs")
