...
from .efficientnet_pytorch_custom import EfficientNet
...
def load_model(device, model_filename):
    """
    Loads the pretrained CrossEfficientViT model.
    
    Args:
        device (torch.device): The device to load the model onto.
        model_filename (str): Path to the model weights file.
        config_path (str): Path to the configuration file.
    
    Returns:
        model (torch.nn.Module): The loaded model in evaluation mode.
    """
    with open("models/cross_efficient_vit.yaml", 'r') as f:
...
def run_inference(model, input_tensor, device):
    """
    Runs inference on the input tensor and returns the average probability for the 'Fake' class.
    
    Args:
        model (torch.nn.Module): The loaded CrossEfficientViT model.
